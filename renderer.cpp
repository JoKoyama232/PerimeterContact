...
{
	XMFLOAT4	pos[SHADOW_MAX];
	XMFLOAT4	towards[SHADOW_MAX];
	XMFLOAT4	up[SHADOW_MAX];
...
static ID3D11RasterizerState* g_RasterStateWireFrame;


static MATERIAL_CBUFFER	g_Material;
static LIGHT_CBUFFER	g_Light;
static FOG_CBUFFER		g_Fog;
static SHADOW_CBUFFER	g_Shadow;

static FUCHI			g_Fuchi;

//シャドウ用
static ID3D11Texture2D* m_shadowMap = NULL;
static ID3D11DepthStencilView* m_shadowDepthView = NULL;
static ID3D11ShaderResourceView* m_shadowResourceView = NULL;
static ID3D11SamplerState* m_comparisonSampler_point = NULL;
...
void SetCullingMode(RASTERIZER_MODE cm)
{
	switch (cm)
	{
	case CULL_MODE_NONE:
	...
	case CULL_MODE_BACK:
		...
		break;
	case CULL_MODE_WIREFRAME:
		g_ImmediateContext->RSSetState(g_RasterStateWireFrame);
		break;
	}
}
...
//{
//	GetDeviceContext()->UpdateSubresource(g_ShadowBuffer, 0, NULL, &g_Shadow, 0, 0);
//}
//
...
//void setShadow(int index, SHADOW* pShadow) 
//{
//	g_Shadow.pos[index] = XMFLOAT4(pShadow->pos.x, pShadow->pos.x, pShadow->pos.x, );
//}
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	rd.CullMode = D3D11_CULL_NONE;
	g_D3DDevice->CreateRasterizerState(&rd, &g_RasterStateWireFrame);
	...
	//シャドウ初期化
	ZeroMemory(&g_Shadow, sizeof(SHADOW_CBUFFER));
	//SetShadowBuffer();
...
void Clear(void)
{
	...
	//g_ImmediateContext->ClearRenderTargetView(m_deviceResources->GetBackBufferRenderTargetView(), DirectX::Colors::CornflowerBlue);
	//g_ImmediateContext->ClearDepthStencilView(m_shadowDepthView.Get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
}
