...
// プロトタイプ宣言
//*****************************************************************************
void LoadObj( char *FileName, MODEL *Model, VERTEX_DATA* Points);
...
// 初期化処理
//=============================================================================
void LoadModel(char* FileName, DX11_MODEL* Model, bool loadHitbox, VERTEX_DATA* Points)
{
	MODEL model;
	VERTEX_DATA points;

	LoadObj(FileName, &model,&points);
	...
	if (loadHitbox) {
		Points->VertexArray = new XMFLOAT3[points.VertexNum];
		Points->VertexNum = points.VertexNum;

		for (unsigned short i = 0; i < points.VertexNum; i++) {
			Points->VertexArray[i] = points.VertexArray[i];
		}
	}
	delete[] points.VertexArray;
...
void LoadObj( char *FileName, MODEL *Model ,VERTEX_DATA *points)
{
	...
	if( sc != 0 ) Model->SubsetArray[ sc - 1 ].IndexNum = ic - Model->SubsetArray[ sc - 1 ].StartIndex;

	points->VertexArray = new XMFLOAT3[positionNum];
	for (int i =0; i < positionNum; i++) {
		points->VertexArray[i] = positionArray[i];
	}
	points->VertexNum = positionNum;
