...
typedef struct
{
	...
	XMFLOAT3		scale;			// スケール
	XMFLOAT3		move;			// 移動量
	XMFLOAT3		deltaMove;		// 移動量の変化量
	MATERIAL		material;		// マテリアル
	float			fSizeX;			// 幅
	float			fSizeY;			// 高さ
	int				nIdxShadow;		// 影ID
	int				nIdxTexture;	// テクスチャID
	int				nLife;			// 寿命
...
HRESULT InitParticle(void)
{
	...
	for(int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		...
		g_aParticle[nCntParticle].deltaMove = XMFLOAT3(1.0f, 1.0f, 1.0f);
		...
		g_aParticle[nCntParticle].nIdxTexture = 0;
...
// パーティクルの発生処理
//=============================================================================
int SetParticle(XMFLOAT3 pos, XMFLOAT3 move, XMFLOAT3 acceleration, XMFLOAT3 rot,
	XMFLOAT4 col, float fSizeX, float fSizeY,bool shadow,int textureID, int nLife)
{
	int nIdxParticle = -1;
	...
	for(int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		if (!g_aParticle[nCntParticle].bUse)
		{
			...
			g_aParticle[nCntParticle].scale = { fSizeX,fSizeY, 1.0f };
			g_aParticle[nCntParticle].move = move;
			g_aParticle[nCntParticle].deltaMove = acceleration;
			...
			g_aParticle[nCntParticle].nIdxTexture = textureID;
